...
from tkinter import messagebox  # Importing messagebox for showing dialog boxes if needed
...
def translate_text():
    ...
    if ask == "W" or ask == "w":  # Check if translation is for a word
        ...
        if text in dictionary:  # Check if word is in the dictionary
            ...
            lsi = text.split()  # Split the text into words
            for i in lsi:
...
# Create title label
title_label = tk.Label(root, text="Language Translator", font=("Blackadder ITC", 16))
...
output_display = tk.Label(root, textvariable=output_text, width=50, height=4, relief="solid")
